...
initialize_fontconfig() {
    export FONT_CONFIG_CACHE="/c/users/wareya/bogus_fcfg/"
...
generate_font_image() {
    ...
    common_args+=" --fonts_dir=${FONTS_DIR} --strip_unrenderable_words=false"
    common_args+=" --leading=${LEADING}"
    common_args+=" --char_spacing=${CHAR_SPACING} --exposure=${EXPOSURE}"
    if [[ ${#TEXTS[@]} -eq 0 ]]; then
        common_args+=" --outputbase=${outbase} --max_pages=100"
